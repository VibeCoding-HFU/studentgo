...
export function useScheduleController(): ScheduleViewModel {
  ...
  const loadOptions = useCallback(async () => {
    const nextOptions = await fetchImportOptions();
    if (nextOptions) {
  ...
  const loadGroups = useCallback(async (nextFaculty: FacultyOption | null, nextSemester: Option | null) => {
    ...
    const nextOptions = await fetchImportOptions({ facultyId: nextFaculty.id, semesterId: nextSemester.id });
    if (nextOptions) {
  ...
  const loadSchedule = useCallback(async () => {
    const body = await fetchScheduleDays({ token }, toInputDate(weekStart));
    if (body) {
      setSchedule(await decryptScheduleDays(body.days, user?.email));
    }
  }, [token, user?.email, weekStart]);
  ...
  const loadMeals = useCallback(async () => {
    const nextMeals = await fetchMeals(toInputDate(weekStart));
    if (nextMeals) {
      setMeals(nextMeals);
    }
  }, [weekStart]);
  ...
  const loadAccountStats = useCallback(async () => {
    ...
    setAccountStats(await fetchAccountStats({ token }));
  }, [token]);
  ...
  useEffect(() => {
    ...
    async function searchAccounts() {
      ...
      const users = await searchUsers({ token }, accountQuery);
      if (!cancelled) {
  ...
  const loadImportCourses = useCallback(async () => {
    ...
    try {
      const result = await fetchImportCourses({ token }, importPayload());
  ...
  async function importSchedule() {
    ...
    try {
      result = await importScheduleRequest({ token }, importPayload());
    } catch {
  ...
  async function importCourse(course: CourseOption) {
    ...
    let result: { count: number; monthStart?: string };
    try {
      result = await importCourseRequest({ token }, {
        ...importPayload(),
  ...
  async function removeImportedCourse(lesson: Lesson) {
    ...
    try {
      const result = await removeImportedCourseRequest({ token }, {
  ...
  async function addLesson() {
    ...
    try {
      const response = await createLesson({ token }, body);
  ...
  async function updatePersonalLesson() {
    ...
    try {
      ...
      const response = await updateLesson({ token }, editingLesson.id, body);
  ...
  async function deletePersonalLesson() {
    ...
    const response = await deleteLesson({ token }, editingLesson.id);
  ...
  async function toggleLessonVisit(lesson: Lesson) {
    ...
    const response = await setLessonVisited({ token }, lesson.id, toInputDate(selectedDate));
  ...
  async function toggleModuleActive(lesson: Lesson) {
    ...
    const response = await setModulePreference({ token }, lesson.moduleKey, lesson.isModuleActive === false);
